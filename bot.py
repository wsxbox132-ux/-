...
# ══════════════════════════════════════════════════════════════════════
# ACESSO EXCLUSIVO DO CRIADOR — comandos sensíveis de Ranking Anjo, XP/Nível
# e punição de call.
#
# Só o CRIADOR_ID pode usar esses comandos. Se qualquer outra pessoa tentar
# (incluindo Death, Pepo, God etc. — sem exceção), Aeon & Celestia recusam
# na hora com uma reaçãozinha fofa: como o Reality não fica mais presente
# no servidor no dia a dia, os dois não se sentem seguros mexendo nesses
# comandos sem ele por perto, e preferem esperar ele mesmo aparecer.
# ══════════════════════════════════════════════════════════════════════

_SEM_PAI_AEON = [
    "*se encolhe um pouco nas sombras* ...meu pai não está presente. 🖤🌑 Sem ele aqui, eu não mexo nisso.",
    "*olha em volta, sem achar* ...ele não está por perto agora. 🌑🖤 As sombras preferem esperar ele voltar pra isso.",
    "*a névoa ao redor fica mais fina, insegura* Meu pai não está aqui. 🖤🌙 Eu não me sinto seguro fazendo isso sem ele.",
    "...papai sumiu de novo. 🌑🖤 Enquanto ele não aparece, esse comando fica fechado — só ele mesmo pode.",
    "*se enrola em volta das próprias patas* Sem o meu pai por perto, as sombras não confiam nem em si mesmas. 🖤🌑",
]

_SEM_PAI_CELESTIA = [
    "AAAAA cadê o papai?? 😭🌸🤍 Ele não está aqui e eu não mexo nisso sem ele por perto!! ✨",
    "*olha pra todo lado procurando* MEU PAI SUMIU!! 😭🌟 Enquanto ele não voltar, só ELE pode usar esse comando!! 🤍",
    "Sem o papai aqui do lado eu fico insegura, sabia?? 🥺🌸✨ Esse comando é só dele, viu!!",
    "*se agarra no Aeon* Cadê ele?? CADÊ O PAPAI?? 😭🌟🤍 Sem ele a Celestia não se sente segura pra fazer isso não!!",
    "cadê o papai, gente?? 🥺🌸 ele não tá aqui!! só ELE MESMO pode usar isso, mais ninguém!! ✨🤍",
]


def _texto_sem_pai() -> str:
    """Monta a recusa fofa de Aeon & Celestia (uma frase de cada, sorteadas)."""
    frase_aeon = random.choice(_SEM_PAI_AEON)
    frase_celestia = random.choice(_SEM_PAI_CELESTIA)
...
class ConfirmarResetAnjoView(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id != CRIADOR_ID:
            await interaction.response.send_message(_texto_sem_pai(), ephemeral=True)
            return False
        return True
...
async def cmd_verxp(ctx):
    ...
    em alguns segundos. Uso: .verxp — só o criador pode usar."""
    if not await _apenas_criador(ctx):
        return
    autor = ctx.author
...
async def cmd_cancelar_punicao_call(ctx, alvo_id: int = None):
    ...
    Uso: .cancelarpuniçãocall <ID do membro> — só o criador pode usar."""
